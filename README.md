...
- **Commission & reports (M5)** — as `anna@demo.is` (ADMIN): *Stillingar* shows the plan usage meter and the company's default söluþóknun scheme (2,2% + fixed fees, editable — fixed %, tiered brackets, or flat + %). Each listing can override the scheme (Söluþóknun card, sidebar) and set agent split percentages in the *Sölumenn* panel (blank = the primary agent gets 100%). Completing a sale (entering *Afsal / Lokið*) freezes a CommissionRecord — see **Heiðarvegur 5** for a frozen record with a 60/40 split. *Skýrslur* (ADMIN-only nav) shows earned commission per month (bar chart) and per agent plus a pipeline forecast for stages 3–6, each exportable as CSV (semicolon-delimited for Icelandic Excel). The söluyfirlit's söluþóknun disclosure is generated from the applicable scheme unless explicit text is set on the listing.
- **Plan limits (M5)** — moving a listing into the active band (*Í sölu* … *Afhending*) beyond the plan's cap is blocked with an upgrade prompt (try it: lower the demo tenant's plan limit under `/admin` as superadmin). ADMINs get a warning email (Mailpit) when usage crosses 90%.
- *(M6)* Handsal Bílar scaffold — the final milestone.
